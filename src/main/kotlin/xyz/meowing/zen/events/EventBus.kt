...
object EventBus {
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        if (event.phase == TickEvent.Phase.START) post(xyz.meowing.zen.events.TickEvent.Client())
    }
    ...
    fun onWorldLoad(event: WorldEvent.Load) {
        if (xyz.meowing.zen.events.WorldEvent.Change.shouldPost()) {
            post(xyz.meowing.zen.events.WorldEvent.Change(event.world))
        }
    }
    ...
    fun onMouse(event: MouseEvent) {
        when {
            event.button != -1 && event.buttonstate -> post(xyz.meowing.zen.events.MouseEvent.Click(event))
            event.button != -1 && !event.buttonstate -> post(xyz.meowing.zen.events.MouseEvent.Release(event))
            event.dwheel != 0 -> post(xyz.meowing.zen.events.MouseEvent.Scroll(event))
            event.dx != 0 || event.dy != 0 -> post(xyz.meowing.zen.events.MouseEvent.Move(event))
        }
    }
    ...
    fun onPacketReceived(packet: Packet<*>): Boolean {
        ...
        return when (packet) {
            is S32PacketConfirmTransaction -> {
                ...
                else post(xyz.meowing.zen.events.TickEvent.Server())
            }
            is S02PacketChat -> {
