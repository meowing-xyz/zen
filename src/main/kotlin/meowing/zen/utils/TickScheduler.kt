...
object TickScheduler {
    ...
    fun onClientTick(event: TickEvent.ClientTickEvent) {
        ...
        currentClientTick++
        while (clientTaskQueue.peek()?.let { currentClientTick >= it.executeTick } == true) clientTaskQueue.poll()
            ?.action()
    }
    ...
    fun onServerTick(event: ServerTickEvent) {
        currentServerTick++
        while (serverTaskQueue.peek()?.let { currentServerTick >= it.executeTick } == true) serverTaskQueue.poll()
            ?.action()
    }
